...
VEL_FRACTION = 100#0.005
TIME_STEP = 100#1000000
...
def col_bubble(vel_fraction, time_step, disc_radius_scaled, scale, loc = [0, 0, 0, 0]):
    """
    Simulate expanding colonisation bubble regarding to its arguments

    Arguments:
    velocity   - velocity as fraction of light
    loc   - center of colonisation bubble
    time_step  - step for bubble expansion visualisation
    """
    c.configure(scrollregion=(-500, -400, 500, 400))
#   print velocity as fraction
    c.create_text(-455, -360, fill='red', anchor='w',
                  text='velocity as fraction of light = {} LY'.format(VEL_FRACTION))
    i = 0
    expend_rad = 0
    current_step = time_step
    while expend_rad != disc_radius_scaled:
        time.sleep(1)  # delay before posting new expansion circle
        expend_rad = current_step*vel_fraction
        rad = round(expend_rad/scale)
#       determine distance to the galaxy edge
        if loc[2]+rad >= scale:
            break
        c.create_text(-455, -330 + i, fill='red', anchor='w',
                      text='Years = {:,} '.format(current_step))
#       draw circle according to expend_rad
        c.create_oval(loc[0]-rad, loc[1]-rad, loc[2]+rad, loc[3]+rad, fill='', outline='red')
        current_step += time_step
        i += 30
    c.create_text(loc[0], loc[1], fill='red', anchor='w',
...
def main():
    ...
    col_bubble(VEL_FRACTION, TIME_STEP, disc_radius_scaled, SCALE, [115, 75, 116, 76])
    # run tkinter loop
    root.mainloop()
