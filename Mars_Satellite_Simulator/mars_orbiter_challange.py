...
class Satellite(pg.sprite.Sprite):
    ...
    def check_keys(self):
        ...
        if 0 >= self.x or self.x >= 400 and 0 >= self.y or self.y >= 100:
...
class Comet(pg.sprite.Sprite):
    ...
    def update(self):
        if self.x < 0 or self.x > 640 or self.y < 0 or self.y > 800:
            self.kill()
        else:
            self.fly()
            self.rect.center = (self.x, self.y)
...
def box_label(screen, text, dimensions, colour = BLACK):
    ...
    label = readout_font.render(text, True, colour)
...
def main():
    ...
    background = pg.image.load('title_screen.png').convert()
    ...
    comet = Comet(background)
    comet_sprite = pg.sprite.Group(comet)
    help = Help('help.png', [400, 200])
    help_sprite = pg.sprite.Group(help)
    ...
    comet.stpoint_set(random.randint(1, 4))
    ...
    while running:
        ...
        for event in pg.event.get():
            if event.type == pg.QUIT:  # close window
                ...
                screen = pg.display.set_mode((800, 640))  # exit full screen
                ...
                # print("call guide")
                screen.blit(help.image, help.rect)
        ...
        instruct_label(screen, instruct_text2, WHITE, 570, 488)
        ...
        if event.type == pg.KEYDOWN and event.key == pg.K_h:
            instruct_font = pg.font.SysFont(None, 25)
            tit_label1 = instruct_font.render('Advantages: ', True, RED)
            screen.blit(tit_label1, (250, 200))
            instruct_label(screen, advantages_text, RED, 250, 222)
            help_sprite.update()
            help_sprite.draw(screen)
            tit_label2 = instruct_font.render('Disadvantages: ', True, RED)
            screen.blit(tit_label2, (250, 300))
            instruct_label(screen, disadvantages_text, RED, 250, 322)
            help_sprite.update()
            help_sprite.draw(screen)
        pg.display.flip()
