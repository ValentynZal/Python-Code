...
def select(population, to_retain):
    ...
    members_per_sex = (len(sorted_population)-20)//2
    females = sorted_population[:members_per_sex+16]
    males = sorted_population[16+members_per_sex:]
    selected_females = females[-16:]
    selected_males = males[-4:]
    return selected_males, selected_females

def list_extend(list, times=3):
    temp = list
...
def main():
    ...
    while popl_fitness < 1 and generations < GENERATION_LIMIT:
        ...
        selected_males_ex = list_extend(selected_males)
        children = breed(selected_males_ex, selected_females, LITTER_SIZE)
